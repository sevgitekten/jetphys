...
void drawDagostini(string type) {
  ...
  TCanvas *c1 = new TCanvas("c1","c1",1200,800);   // 800
  //c1->SetTopMargin(0.20);
  c1->Divide(3,2);//,0.1,0.00); 3,2
  ...
  TLegend *leg = new TLegend(0.45,0.04,0.65,0.27,"","brNDC");
  ...
  for (int iy = 0; iy != ny; ++iy) {
    ...
    //l->DrawLine(jp::unfptminnlo,0.1,jp::unfptminnlo,1.15); // minimum pt in unfolding

    l->SetLineStyle(kDashed);
    l->DrawLine(jp::unfptminreco,0.1,jp::unfptminreco,1.15); 
    ...
    tex->SetNDC(kFALSE);
    tex->DrawLatex(80, 0.25, y1==0 ? "|y|<0.5" : Form("%1.1f<|y|<%1.1f",y1,y2));
    ...
    tex->DrawLatex(70,0.91, y1==0 ? "|y|<0.5" : Form("%1.1f<|y|<%1.1f",y1,y2));
